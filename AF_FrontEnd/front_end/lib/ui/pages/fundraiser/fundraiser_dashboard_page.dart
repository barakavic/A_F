...
class _FundraiserDashboardPageState extends ConsumerState<FundraiserDashboardPage> {
  ...
  Widget _buildStatsGrid(WidgetRef ref) {
    ...
    return statsAsync.when(
      data: (stats) => Row(
        children: [
          Expanded(child: _buildSimpleStatCard("Total Raised", "KES ${currencyFormatter.format(stats.totalRaised)}")),
          const SizedBox(width: 16),
          Expanded(child: _buildSimpleStatCard("Active Phases", stats.activePhasesCount.toString())),
        ],
      ),
      loading: () => Row(
        children: [
          Expanded(child: _buildSimpleStatCard("Total Raised", "...")),
          const SizedBox(width: 16),
          Expanded(child: _buildSimpleStatCard("Active Phases", "...")),
        ],
      ),
      error: (err, stack) => Row(
        children: [
          Expanded(child: _buildSimpleStatCard("Total Raised", "Error")),
          const SizedBox(width: 16),
          Expanded(child: _buildSimpleStatCard("Active Phases", "Error")),
        ],
      ),
    );
  }
